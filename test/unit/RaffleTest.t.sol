...
contract RaffleTest is Test {
        ...
        raffleEnteredAndTimePassed
    {
        ...
        // @notice real chainだとchainlink nodeしかアクセスできない！
        VRFCoordinatorV2Mock(vrfCoordinator).fulfillRandomWords(randomRequestId, address(raffle));
    }

    function testFulfillRandomWordsPicksAWinnerAndSendsMoney() public raffleEnteredAndTimePassed {
        // Arrange
        ...
        bytes32 winner = emittedEvents[0].topics[1];

        // Assert
        // it is better to one assert per one test
        assertEq(uint256(raffle.getRaffleState()), 0);
        assert(raffle.getRecentWinner() != address(0));
        assertEq(raffle.getLengthOfPlayers(), 0);
        assert(previousTimeStamp < raffle.getLastTimeStamp());
        assert(winner != 0);
        assertEq(raffle.getRecentWinner().balance, STARTING_USER_BALANCE + prize - entranceFee);
    }
}
